...
async function handler(req, res) {
  ...
  try {
    ...
    if (!currentDonationId || currentDonationId === 'null') {
      // No donation currently playing, just clear flags
      ...
      return res.status(200).json({ 
        success: true, 
        message: 'No current donation, flags cleared' 
      });
    }

    // Get the donation from queue
    const queueResult = await db.query('SELECT * FROM DonationQueue WHERE id = ?', [parseInt(currentDonationId)]);
    ...
    console.log('Donation sequence completed:', {
    ...
    console.error('Complete current donation error:', error);
