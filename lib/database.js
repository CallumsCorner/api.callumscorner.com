...
const db = {
  ...
  async removeDonationFromQueueAndStore(id) {
    const donation = await this.query('SELECT * FROM DonationQueue WHERE id = ?', [id]);
    if (!donation || donation.length === 0) return null;
    const d = donation[0];
    await this.query(
      `INSERT INTO RemovedDonations (original_id, order_id, name, amount, message, originalMessage, originalName, payer_id, is_replay, filter_matched_words, name_was_filtered, message_was_filtered, filter_reasoning, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [d.id, d.order_id, d.name, d.amount, d.message, d.originalMessage, d.originalName, d.payer_id, d.is_replay, d.filter_matched_words, d.name_was_filtered, d.message_was_filtered, d.filter_reasoning, d.created_at]
    );
    await this.query('DELETE FROM DonationQueue WHERE id = ?', [id]);
    return d;
  },
  ...
  async removeMediaFromQueueAndStore(id) {
    const media = await this.query('SELECT * FROM MediaQueue WHERE id = ?', [id]);
    if (!media || media.length === 0) return null;
    const m = media[0];
    await this.query(
      `INSERT INTO RemovedMedia (original_id, order_id, donor_name, media_url, media_start_time, video_title, video_thumbnail, video_duration, payer_id, is_replay, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [m.id, m.order_id, m.donor_name, m.media_url, m.media_start_time, m.video_title, m.video_thumbnail, m.video_duration, m.payer_id, m.is_replay, m.created_at]
    );
    await this.query('DELETE FROM MediaQueue WHERE id = ?', [id]);
    return m;
  },
  ...
  async getDonationFromQueue(id) {
    const result = await this.query('SELECT * FROM DonationQueue WHERE id = ?', [id]);
    return result[0] || null;
  },

  async getMediaFromQueue(id) {
    const result = await this.query('SELECT * FROM MediaQueue WHERE id = ?', [id]);
    return result[0] || null;
  },

  async updateDonation(table, id, { name, message }) {
    if (name !== undefined && message !== undefined) {
      await this.query(`UPDATE ${table} SET name = ?, message = ? WHERE id = ?`, [name, message, id]);
    } else if (name !== undefined) {
      await this.query(`UPDATE ${table} SET name = ? WHERE id = ?`, [name, id]);
    } else if (message !== undefined) {
      await this.query(`UPDATE ${table} SET message = ? WHERE id = ?`, [message, id]);
    } else {
      return null;
    }
...
db.addDonationToQueue = async function(donationData) {
  return await this.query(`
    INSERT INTO DonationQueue (order_id, name, amount, message, originalMessage, originalName, payer_id, is_replay, filter_matched_words, name_was_filtered, message_was_filtered, filter_reasoning)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [
    ...
    donationData.filter_matched_words || null,
    donationData.name_was_filtered || false,
    donationData.message_was_filtered || false,
    donationData.filter_reasoning || null
  ]);
};

db.addDonationToHistory = async function(donationData) {
  return await this.query(`
    INSERT INTO DonationHistory (order_id, name, amount, message, originalMessage, originalName, payer_id, is_replay, filter_matched_words, name_was_filtered, message_was_filtered, filter_reasoning)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [
    donationData.order_id,
    donationData.name,
